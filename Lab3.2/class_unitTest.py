...
class TESTAGENDA(unittest.TestCase):
	...
	def test_happyPath(self):
		...
		#adding a record, trying to add a fake record, finding a valid record, finding an invalid record
		#deleting a valid record, deleting an invalid record and displaying all records at once 
	...
	def test_boundary(self):
		#B: Testing boundaries, verfiying first that the file recording the users doesn't exist 
		#Testing that all actions doesn't work when the file doesn´t exist, then after adding a record
		...
		if os.name == 'posix':
			...
			with self.assertRaises(Exception):
				recordForTestingB.DisplayAll("UserForTesting")
